...
def __create_sizes_and_ingredients_and_beverages(ingredients: list, sizes: list, beverages: list):
...
def test_create(app, ingredients, size, beverages, client_data):
    created_size, created_ingredients, created_beverages = __create_sizes_and_ingredients_and_beverages(ingredients, [size], beverages)
    ...
    for param, value in order.items():
        ...
        beverages_in_order = set(item['beverage']['_id'] for item in created_order['beverage'])
        pytest.assume(not ingredients_in_detail.difference(ingredient_ids))
        pytest.assume(not beverages_in_order.difference(beverage_ids))
...
def test_get_all(app, ingredients, sizes, beverages, client_data):
    created_sizes, created_ingredients, created_beverages = __create_sizes_and_ingredients_and_beverages(ingredients, sizes, beverages)
    created_orders = []
    for _ in range(5):
